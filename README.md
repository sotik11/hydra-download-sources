...
**Запуск лише локальний.** Обидва сайти віддають **403 на IP дата-центрів**
(перевірено 2026-07-25: з раннера GitHub Actions — 403, з домашнього IP — повні
дані). Тому хмарний крон для скрейпу неможливий; `refresh_local.sh` ганяє
**обидва джерела** на резидентному IP і чіпляється до спільної Windows-задачі
«Hydra localization refresh» (обгортка `C:\temp\claude\refresh_all.sh`: спочатку
локалізації, потім ці джерела). Скрипт: pull → кожен генератор (інкремент, з
guard «<50% → відкат») → один commit/push у `main` → Windows-toast.
